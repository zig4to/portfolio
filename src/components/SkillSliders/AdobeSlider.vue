<template>
    ...
    <div ref="rectangleContainer" class="flex flex-col items-center justify-end">
      ...
      <div class="h-[200px] w-[20px] flex items-end justify-start overflow-hidden">
        <div ref="rectangle" class="h-[0px] w-[20px] bg-[#A34040] opacity-85 transition-all duration-[3000ms] rounded-sm"></div>
      </div>

      <!-- Logo Below Progress Bar -->
      <div class="mt-4">
        <img src="/src/assets/logos/skillslider/adobe.svg" alt="Logo" class="w-7 h-7 mt-5 object-contain">
      </div>
    </div>
  </template>
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              setTimeout(() => {
                rectangle.value.classList.add("h-[50%]"); // Expands after delay
              }, 1200); // Delay for 2000ms (1 second)
            }
          });
        },
